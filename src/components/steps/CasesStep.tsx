...
const emptyCase: CaseStudy = { title: '', summary: '', outcome_metrics: '' };
...
export function CasesStep({ caseStudies, onChange }: CasesStepProps) {
  ...
  const addCase = () => { onChange([...caseStudies, { ...emptyCase, case_id: crypto.randomUUID() }]); setExpandedIndex(caseStudies.length); };
  ...
  return (
    <FormCard title="Case Studies" description="Showcase successful projects and outcomes.">
      <div className="space-y-4">
          ...
          <div key={cs.case_id || index} className="border border-border rounded-xl overflow-hidden bg-background">
            <button type="button" onClick={() => setExpandedIndex(expandedIndex === index ? null : index)}
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium text-foreground">{cs.title || `Case Study ${index + 1}`}</span>
              </div>
            ...
            <div className={cn("transition-all duration-300 overflow-hidden", expandedIndex === index ? "max-h-[500px] opacity-100" : "max-h-0 opacity-0")}>
              <div className="p-4 pt-0 space-y-4 border-t border-border">
                <FormField label="Title" required>
                  <Input placeholder="Project Success Story" value={cs.title || ''} onChange={(e) => updateCase(index, 'title', e.target.value)} />
                </FormField>
                <FormField label="Summary">
                  <Textarea placeholder="Describe the project..." value={cs.summary || ''} onChange={(e) => updateCase(index, 'summary', e.target.value)} />
                </FormField>
                <FormField label="Outcome / Metrics">
                  <Input placeholder="50% increase in sales" value={cs.outcome_metrics || ''} onChange={(e) => updateCase(index, 'outcome_metrics', e.target.value)} />
                </FormField>
                <div className="flex justify-end">
                  <Button type="button" variant="ghost" size="sm" onClick={() => removeCase(index)} className="text-destructive hover:text-destructive">
        ...
        <Button type="button" variant="outline" onClick={addCase} className="w-full h-12 border-dashed">
