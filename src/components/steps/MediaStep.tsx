...
const emptyMention: MediaMention = { title: '', publications: '', date: '', url: '' };
  ...
  return (
    <FormCard title="Media Mentions" description="Add press coverage and media appearances.">
      <div className="space-y-4">
          ...
          <div key={index} className="border border-border rounded-xl overflow-hidden bg-background">
            <button type="button" onClick={() => setExpandedIndex(expandedIndex === index ? null : index)}
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium text-foreground">{mention.title || `Mention ${index + 1}`}</span>
              </div>
              ...
              <div className="p-4 pt-0 space-y-4 border-t border-border">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormField label="Title" required>
                    <Input placeholder="Featured in Forbes" value={mention.title || ''} onChange={(e) => updateMention(index, 'title', e.target.value)} />
                  </FormField>
                  <FormField label="Publication">
                    <Input placeholder="Forbes" value={mention.publications || ''} onChange={(e) => updateMention(index, 'publications', e.target.value)} />
                  </FormField>
                  <FormField label="Date">
                    <Input type="date" value={mention.date || ''} onChange={(e) => updateMention(index, 'date', e.target.value)} />
                  </FormField>
                  <FormField label="URL">
                    <Input type="url" placeholder="https://..." value={mention.url || ''} onChange={(e) => updateMention(index, 'url', e.target.value)} />
                  </FormField>
                </div>
                <div className="flex justify-end">
                  <Button type="button" variant="ghost" size="sm" onClick={() => removeMention(index)} className="text-destructive hover:text-destructive">
        ...
        <Button type="button" variant="outline" onClick={addMention} className="w-full h-12 border-dashed">
          <Plus className="w-5 h-5 mr-2" />Add Media Mention
        </Button>
      </div>
    </FormCard>
  );
