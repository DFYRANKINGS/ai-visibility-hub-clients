...
export function FAQsStep({ faqs, onChange }: FAQsStepProps) {
  ...
  return (
    <FormCard title="FAQs" description="Frequently asked questions help AI provide accurate answers about your business.">
      <div className="space-y-4">
          ...
          <div key={index} className="border border-border rounded-xl overflow-hidden bg-background">
            <button type="button" onClick={() => setExpandedIndex(expandedIndex === index ? null : index)}
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium text-foreground truncate pr-4">{faq.question || `FAQ ${index + 1}`}</span>
              </div>
            ...
            <div className={cn("transition-all duration-300 overflow-hidden", expandedIndex === index ? "max-h-[600px] opacity-100" : "max-h-0 opacity-0")}>
              <div className="p-4 pt-0 space-y-4 border-t border-border">
                <FormField label="Question" required>
                  <Input placeholder="What makes your service unique?" value={faq.question} onChange={(e) => updateFAQ(index, 'question', e.target.value)} />
                </FormField>
                <FormField label="Answer" required>
                  <Textarea placeholder="Provide a detailed answer..." value={faq.answer} onChange={(e) => updateFAQ(index, 'answer', e.target.value)} className="h-24" />
                </FormField>
                <FormField label="Keywords" hint="Comma-separated keywords for search">
                  <Input placeholder="returns, refunds, policy" value={faq.keywords || ''} onChange={(e) => updateFAQ(index, 'keywords', e.target.value)} />
                </FormField>
        ...
        <Button type="button" variant="outline" onClick={addFAQ} className="w-full h-12 border-dashed">
