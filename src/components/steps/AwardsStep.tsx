...
const emptyAward: Award = { name: '', issuer: '', date_awarded: '', award_url: '' };
...
export function AwardsStep({ awards, onChange }: AwardsStepProps) {
  ...
  return (
    <FormCard title="Awards & Recognition" description="Showcase your achievements and credentials.">
      <div className="space-y-4">
              ...
              <div className="p-4 pt-0 space-y-4 border-t border-border">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormField label="Award Name" required>
                    <Input placeholder="Best in Industry" value={award.name || ''} onChange={(e) => updateAward(index, 'name', e.target.value)} />
                  </FormField>
                  <FormField label="Issuer">
                    <Input placeholder="Industry Association" value={award.issuer || ''} onChange={(e) => updateAward(index, 'issuer', e.target.value)} />
                  </FormField>
                  <FormField label="Date Awarded">
                    <Input type="date" value={award.date_awarded || ''} onChange={(e) => updateAward(index, 'date_awarded', e.target.value)} />
                  </FormField>
                  <FormField label="URL">
                    <Input type="url" placeholder="https://..." value={award.award_url || ''} onChange={(e) => updateAward(index, 'award_url', e.target.value)} />
                  </FormField>
                </div>
                <div className="flex justify-end">
                  <Button type="button" variant="ghost" size="sm" onClick={() => removeAward(index)} className="text-destructive hover:text-destructive">
        ...
        <Button type="button" variant="outline" onClick={addAward} className="w-full h-12 border-dashed">
          <Plus className="w-5 h-5 mr-2" />Add Award
        </Button>
      </div>
    </FormCard>
  );
